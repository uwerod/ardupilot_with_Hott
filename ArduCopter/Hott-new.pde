...
struct HOTT_GAM_MSG {
	...
	int8_t batt2_H;				//#16
	int8_t temperature1;			//#17 temperature 1. offset of 20. a value of 20 = 0Â°C
	int8_t temperature2;			//#18 temperature 2. offset of 20. a value of 20 = 0Â°C
...
struct HOTT_VARIO_MSG {
	...
	int8_t compass_direction;		//#42 Compass heading in 2Â° steps. 1 = 2Â°
	int8_t version;				//#43 version number TODO: more info?
...
struct HOTT_GPS_MSG {
  ...
  int8_t angle_compass; //#32 angle in 2degree steps. 1 = 2Â°, 255 = - 2Â° (1 int8_t) North = 0Â°
...
void _hott_update_gam_msg() {
	...
	hott_gam_msg.temperature2 = 20; // 0Â°C
	...
	(int &)hott_gam_msg.current_L = battery.current_amps()*10;
	(int &)hott_gam_msg.main_voltage_L = (int)(battery.voltage() * 10.0);
	(int &)hott_gam_msg.batt_cap_L = battery.current_total_mah() / 10;
	hott_gam_msg.fuel_procent = battery.capacity_remaining_pct();	// my fuel are electrons :)
	(int &)hott_gam_msg.speed_L = (int)((float)(g_gps->ground_speed_cm * 0.036));
...
void _hott_update_eam_msg() {
	...
	hott_eam_msg.temp2 = 20;	//0Â°
	(int &)hott_eam_msg.altitude_L = (int)((current_loc.alt - home.alt) / 100)+500;
	(int &)hott_eam_msg.current_L = battery.current_amps()*10; 
	(int &)hott_eam_msg.main_voltage_L = (int)(battery.voltage() * 10.0);
	(int &)hott_eam_msg.batt_cap_L = battery.current_total_mah() / 10;
	(int &)hott_eam_msg.speed_L = (int)((float)(g_gps->ground_speed_cm * 0.036));
...
void _hott_update_gps_msg() { 
  ...
  (int &)hott_gps_msg.msl_altitude_L = (int)g_gps->altitude_cm / 100;  //meters above sea level  
  hott_gps_msg.flight_direction = (int8_t)(g_gps->ground_course_cd / 200);  // in 2* steps
  (int &)hott_gps_msg.gps_speed_L = (int)((float)(g_gps->ground_speed_cm * 0.036));
  ...
  switch(control_mode) {
	case AUTO:
          ...
          {
          	  int32_t dist = wp_nav.get_distance_to_target(); // get_distance_cm(&current_loc, &next_WP);
        ...
        //Display Home direction and distance
        {
          ...
          int32_t dist = pythagorous2(curr.x, curr.y); // get_distance_cm(&current_loc, &home);
          (int &)hott_gps_msg.home_distance_L = dist < 0 ? 0 : dist / 100;
...
const char hott_supersimple_STR[] PROGMEM	= "supsimp";
...
void _hott_update_vario_msg() {
	...
	char *psimpleStr = (char *)hott_normal_STR;
	if (ap.simple_mode == 1) {
	...
	uint8_t len2 = strlen_P(psimpleStr);
	memcpy_P((char*)hott_vario_msg.text_msg, pArmedStr, len);
	memcpy_P((char*)&hott_vario_msg.text_msg[len+1], psimpleStr, len2);
	memcpy_P((char*)&hott_vario_msg.text_msg[len+1+len2+1], hott_flight_mode_strings[control_mode], strlen_P(hott_flight_mode_strings[control_mode]));
}
#endif
...
void _hott_eam_check_mAh() {
	...
	if( (battery.monitoring()== 4) && (20 > battery.capacity_remaining_pct())) {
...
void _hott_eam_check_mainPower() {
	...
	if((millis() > 10000) && (battery.monitoring()!= 0) && (battery.voltage() <  g.fs_batt_voltage)) {
		_hott_alarm_event _hott_ema_alarm_event;
		_hott_ema_alarm_event.alarm_time = 6;	//1sec units
